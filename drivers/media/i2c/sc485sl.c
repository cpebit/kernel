...
static int sc485sl_set_gain_reg(struct sc485sl *sc485sl, u32 gain, int mode)
{
	...
	//according datasheet, fine_dgain max is 0xfe, so we need to check it to avide the dgain
	// overflow, if fine_dgain is odd number, we need to minus 1. The overlfow will lead to
	// sensor ae shading.
	if (fine_dgain % 2 > 0)
		fine_dgain -= 1;
	if (fine_dgain < 0x80)
		fine_dgain = 0x80;
	else if (fine_dgain > 0xfe)
		fine_dgain = 0xfe;
...
static int sc485sl_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_EXPOSURE:
		...
		if (sc485sl->cur_mode->hdr_mode == NO_HDR) {
			/* exposure time as one line as unit */
