...
static const struct  regval __maybe_unused sc635hai_global_2lane_regs[] = {
	{REG_NULL, 0x00},
};
 ...
 * linear: 3200x1800
 * Cleaned_0x0c_SC635HAI_raw_MIPI_27Minput_2Lane_10bit_720Mbps_3200x1800_30fps_20250217.ini
 */
static const struct regval __maybe_unused sc635hai_linear_10_3200x1800_30fps_2lane_regs[] = {
	/* tdoo */
	{REG_NULL, 0x00},
};
...
static const struct sc635hai_mode supported_modes_4lane[] = {
	...
	/* hdr2 30fps */
	// TODO: add hdr2 30fps 4lane
};

static const struct sc635hai_mode __maybe_unused supported_modes_2lane[] = {
	/* TODO: add 2lane modes */
};
			     ...
			     struct preisp_hdrae_exp_s *ae)
{
	...
	// (3033[0], 3e23~3e24) default value is 0x00c4  from reg list
	if (s_exp_time > 184)	// 184 = (3033[0],3e23~3e24) - 13
		s_exp_time = 184;
...
static long sc635hai_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_SET_QUICK_STREAM:
		...
		if (sc635hai->enable_light_ctl) {
			sc635hai->light_param.light_enable = stream;
			light_ctl_write(sc635hai->module_index, &sc635hai->light_param);
		}
	...
	case RKCIS_CMD_FLASH_LIGHT_CTRL:
		dev_info(&sc635hai->client->dev, "set flash light param\n");
		light_param = (struct rk_light_param *)arg;
		if (light_param->light_enable) {
			memcpy(&sc635hai->light_param, light_param,
			       sizeof(struct rk_light_param));
			sc635hai->enable_light_ctl = true;
		} else {
			sc635hai->enable_light_ctl = false;
		}
		ret = light_ctl_write(sc635hai->module_index, light_param);
		break;
	default:
		ret = -ENOIOCTLCMD;
		break;
...
static int __sc635hai_power_on(struct sc635hai *sc635hai)
{
	...
	cam_sw_regulator_bulk_init(sc635hai->cam_sw_inf,
				   SC635HAI_NUM_SUPPLIES, sc635hai->supplies);
...
static int sc635hai_initialize_controls(struct sc635hai *sc635hai)
{
	...
	if (lanes == 2) {
		...
		if (mode->hdr_mode == NO_HDR)
			sc635hai->pixel_rate = v4l2_ctrl_new_std(handler, NULL,
								 V4L2_CID_PIXEL_RATE,
